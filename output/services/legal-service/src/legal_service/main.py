"""FastAPI application entrypoint for Legal Service."""
...
try:
    from legal_service.infrastructure.platform import init_platform
    _has_platform = True
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        from legal_service.integrations.consul import consul_client
...
app = FastAPI(
    title="Legal Service",
    description="Venturestrat legal-service",
    version=settings.service_version,
    lifespan=lifespan,
)

# Add middleware (order matters!)
from legal_service.middleware.security_headers import SecurityHeadersMiddleware
from legal_service.middleware.tenant import TenantContextMiddleware
from legal_service.middleware.rate_limit import RateLimitMiddleware
from legal_service.middleware.observability import ObservabilityMiddleware
...
if settings.tracing_enabled:
    from legal_service.integrations.tracing import setup_tracing
    setup_tracing(app)
...
from legal_service.health import get_detailed_health
...
def metrics():
    ...
    from legal_service.integrations.metrics import metrics_endpoint
...
# =============================================================================
try:
    from legal_service.api.router_registry import register_entity_routers
...
try:
    from legal_service.api.legal_service import router as legal_service_router
    app.include_router(legal_service_router, prefix="/api/v1", tags=["legal_service"])
except ImportError:
    pass

try:
    from legal_service.api.endpoints.documents import router as documents_router
    app.include_router(documents_router, prefix="/api/v1/documents", tags=["document-generation"])
except ImportError:
    pass
